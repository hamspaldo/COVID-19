...
library(pacman)
p_load(dplyr,readr, glue, lubridate, gridExtra, kableExtra, tidyr, ggplot2, cowplot, forcats)
```
...
confirmed <- read_csv("time_series_19-covid-Confirmed.csv") %>%
  ...
  pivot_longer(-country, names_to = "date", values_to = "total") %>%
...
#Checks
write.csv(confirmed, file = "check_confirmed.csv") # there is no double counting of Country.
glimpse(confirmed) # why is this the max dagte: "3/9/20"
max(confirmed$date)
...
recovered <- read_csv("time_series_19-covid-Recovered.csv") %>%
  ...
  pivot_longer(-country, names_to = "date", values_to = "total") %>% #%>%  filter(country=="US")
  mutate(group="Recovered")
  #%>%  filter(country=="Australia")
  #%>%  filter(country=="US")
